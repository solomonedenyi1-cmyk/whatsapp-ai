...
class SqlitePersistenceService {
    ...
    async migrateFromJsonIfNeeded() {
        ...
        const userPreferencesFile = path.join(this.dataDir, 'user_preferences.json');

        const rowCounts = this.db
            .prepare(
                'SELECT (SELECT COUNT(*) FROM conversations) AS conversations, (SELECT COUNT(*) FROM user_preferences) AS preferences'
            )
            .get();

        const hasData =
            (rowCounts?.conversations || 0) > 0 || (rowCounts?.preferences || 0) > 0;
        ...
        const [hasConversations, hasAnalytics, hasPreferences] = await Promise.all([
            fileExists(conversationsFile),
            fileExists(analyticsFile),
            fileExists(userPreferencesFile),
        ]);

        if (!hasConversations && !hasAnalytics && !hasPreferences) {
            return;
        }
        ...
        `);
                ...
                const tx = this.db.transaction((entries) => {
                    for (const [chatId, data] of entries) {
                        if (!chatId || !data || typeof data !== 'object') {
                            continue;
                        }
                        ...
                        insertConversation.run(
                            ...
                            Number.isFinite(data.messageCount) ? data.messageCount : context.length,
                            Number.isFinite(data.participantCount) ? data.participantCount : null,
                            typeof data.lastActivity === 'string' ? data.lastActivity : null,
                            typeof data.mistralConversationId === 'string' ? data.mistralConversationId : null
                        );
                    }
                });
            ...
            if (hasPreferences) {
                const raw = await fs.readFile(userPreferencesFile, 'utf8');
                const parsed = JSON.parse(raw);
        ...
        `);
                ...
                const tx = this.db.transaction((entries) => {
                    for (const [userId, prefs] of entries) {
                        ...
                        const lastUpdated =
                            prefs && typeof prefs === 'object' ? prefs.lastUpdated : null;
            ...
            console.error('❌ Error migrating JSON persistence to SQLite:', error.message);
        }
    }
