...
class AdminService {
  constructor() {
    ...
    console.log('ℹ️  All commands are public (no restrictions)');
  }
  ...
  validateAdminAccess(chatId, command) {
    ...
    // ✅ Since adminCommands is empty, everyone has access
...
module.exports = AdminService;      `🛡️ Acesso negado. Apenas administradores podem usar /${command}.`,
      `⛔ Comando /${command} não disponível para usuários regulares.`,
      `🚫 Você não tem permissão para usar o comando /${command}.`
    ];
